...
int main() {
    ...
    ofstream out_one("results/letters/one.txt");
    ofstream out_two("results/letters/two.txt");
    ofstream out_three("results/letters/three.txt");
    ofstream out_four("results/letters/four.txt");
    ofstream out_five("results/letters/five.txt");

    string alphabet = "abcdefghijklmnopqrstuvwxyz";
    vector<string> guesses = read_guesses();
    vector<string> solutions = read_solutions();

    // 5 tiles, 26 characters
    ...
    for (string guess: guesses) {
        for (string soln: solutions) {
            ...
            for (int i = 0; i < 5; i++) {
                guess_tiles[i][guess[i] - 'a'].push_back(compare_str[i]);
            }
        }
    }
